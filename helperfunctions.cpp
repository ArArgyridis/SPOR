...
double getMapKeys( map<T, U>  k) {

    for (typename map<T,U>::iterator it = k.begin(); it != k.end(); it++  )
        cout << it->first;
...
/*
template <class T>
T getCommonElements(T first, T second) {
...
string genRandomString( const int len) {
    ...
    for (int i = 0; i < len; ++i) {
        //cout <<alphanum.at (    rand() % ( alphanum.length() - 1) );
