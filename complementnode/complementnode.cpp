...
#include "complementnode.hxx"


void ComplementNode::forceNULLMembershipValue() {
    setMembershipValue( COMPLEMENT_NODE_NULL );
    subNodeVector.at( 0 )->forceNULLMembershipValue();
}

ComplementNode::ComplementNode() {
    setNodeType(COMPLEMENT);
    setMembershipValue(COMPLEMENT_NODE_NULL);
}
...
void ComplementNode::resetMembershipValue() {
    setMembershipValue( COMPLEMENT_NODE_NULL );
    subNodeVector.at( 0 )->forceNULLMembershipValue();
}

void ComplementNode::valueEstimation(int curSegment) {
     subNodeVector.at(0)->valueEstimation(curSegment);
    setMembershipValue(  1.0 - subNodeVector.at(0)->getMembershipValue()  );
}
